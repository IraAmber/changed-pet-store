...
import { increaseProductQuantity, decreaseProductQuantity } from '../../store/cart/actions'; // Імпортуємо функції зменшення та збільшення кількості товару у кошику
...
const CardInCart = ({ 
    ...
    orderModalStyles // Використовувати стилі для модального вікна замовлення (булеве значення)
}) => {
    const cardsInCart = useSelector(({ cardsInCart }) => cardsInCart); // Отримуємо дані з Redux за допомогою useSelector
    const currentCard = cardsInCart.find(({ id }) => id === articul); // Знаходимо поточну картку товару у кошику за артикулом
    const dispatch = useDispatch(); // Отримуємо функцію dispatch з Redux для відправлення дій

    const decrementHandler = () => {
        dispatch(decreaseProductQuantity(articul)); // Відправляємо дію на зменшення кількості товару у кошику
    }

    const incrementHandler = () => {
        dispatch(increaseProductQuantity(articul)); // Відправляємо дію на збільшення кількості товару у кошику
    }
...
//     articul: PropTypes.number.isRequired, // Артикул обов'язковий та має бути числом
